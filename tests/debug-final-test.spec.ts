...
test('최종 디버깅 테스트', async ({ page }) => {
  ...
  if (tabSectionCount > 0) {
    ...
    for (let i = 0; i < Math.min(20, spanCount); i++) {
    ...
    // 기타 줄 라벨 확인
    const guitarStringLabels = tabSection.locator('span').filter({ hasText: /^[EBGDAE]$/ })
    const guitarStringCount = await guitarStringLabels.count()
    console.log(`기타 줄 라벨 개수: ${guitarStringCount}`)
  }
  
  // 스크린샷 저장
  await page.screenshot({ path: 'test-results/debug-final-test.png', fullPage: true })
})
